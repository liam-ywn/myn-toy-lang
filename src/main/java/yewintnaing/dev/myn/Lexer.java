...
final class Lexer {
    ...
    private void scanToken() {
        ...
        switch (c) {
            ...
            case '&' -> {
                if (match('&')) { add(TokenType.AND_AND); } else { err("Unexpected '&'"); }
            }
            case '|' -> {
                if (match('|')) { add(TokenType.OR_OR); } else { err("Unexpected '|'"); }
            }
            case ' ', '\r', '\t' -> {
            }
            case '\n' -> {
            ...
            case '"' -> string();
            default -> {
                if (isDigit(c)) { number(); } else if (isAlpha(c)) { identifier(); } else {
